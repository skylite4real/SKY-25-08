...
function handleWheel(event, state, trackpadThreshold, mediaArray, displayCurrentMedia) {
    ...
    if (Math.abs(event.deltaX) > trackpadThreshold) {
...
function displayCurrentMedia(state) {
    const swiperContainer = document.getElementById('mediaSwiper');
    swiperContainer.swiper.slideTo(state.currentIndex, 200); // Added duration for smoother transition
}
...
document.getElementById('postBox').addEventListener('touchmove', (e) => handleMove(e, swipeState, 10, swiperWrapper.children, displayCurrentMedia));
...
document.getElementById('postBox').addEventListener('mousemove', (e) => handleMove(e, swipeState, 10, swiperWrapper.children, displayCurrentMedia));
